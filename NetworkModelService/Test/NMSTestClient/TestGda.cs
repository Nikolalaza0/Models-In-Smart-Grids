...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        public void ComplexMethod()
        {
            try
            {
                // Step 1: Get Power Transformers using GetExtentValues
                List<long> curveIds = GetCurves();

                // Step 2: For each Power Transformer, print its windings to the console using GetRelatedValues
                foreach (long curveId in curveIds)
                {
                    Console.WriteLine($"Curve ID: {curveId}");

                    List<long> curveDatas = GetCurveDatas(curveId);

                    foreach (long curveDataId in curveDatas)
                    {
                        Console.WriteLine($"Curve Data ID: {curveDataId}");
            ...
            {
                string message = $"Printing curves with datas failed. Reason: {e.Message}";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
        }
        ...
        public List<long> GetCurves()
        {
            ModelCode modelCode = ModelCode.CURVE;

            string message = "Getting curves method started.";
            //Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetCurves_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting curves method successfully finished.";
                //Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            {
                message = string.Format("Getting curves method failed for {0}.\n\t{1}", modelCode, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetCurveDatas(long sourceGlobalId)
        {
            Association association = new Association()
            {
                PropertyId = ModelCode.CURVE_CURVEDATAS,
                Type = ModelCode.CURVEDATA
            };

            string message = "Getting curve datas method started.";
            //Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds(ModelCode.CURVEDATA);

                int iteratorId = GdaQueryProxy.GetRelatedValues(sourceGlobalId, properties, association);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetCurveDatas_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting curve datas method successfully finished.";
                //Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting curve datas method  failed for sourceGlobalId = {0} and association (propertyId = {1}, type = {2}). Reason: {3}", sourceGlobalId, association.PropertyId, association.Type, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        //public List<long> ComplexMethod()
        //{
        //    List <long> ids = new List<long>();
        //    List<ResourceDescription> equipments = new List<ResourceDescription>();
        //    ModelCode modelCode = ModelCode.EQUIPMENT;
        //    int iteratorId = 0;
        //    string message = "";
        //    string result = "";
        ...
        //    Association association = new Association()
        //    {
        //        PropertyId = ModelCode.PSR_OUTAGESCHS,
        //        Type = ModelCode.OUTAGESCH
        //    };
        ...
        //                ids.Add(rds[i].Id);
        //                //rds[i].ExportToXml(xmlWriter);
        //                equipments.Add(rds[i]);
        //            }

        //            resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
        //        }
        //        GdaQueryProxy.IteratorClose(iteratorId);
        //        List<ModelCode> properties1 = modelResourcesDesc.GetAllPropertyIds(ModelCode.OUTAGESCH);


        //        foreach (long i in ids)
        //        {
        //            result = GdaQueryProxy.GetRelatedValues(i, properties1, association);
        ...
        //        //message = "Getting equipments method successfully finished.";
        //        //Console.WriteLine(message);
        //        //CommonTrace.WriteTrace(CommonTrace.TraceError, message);
        ...
        //    catch (Exception e)
        //    {
        //        message = string.Format("Getting equipments method failed for {0}.\n\t{1}", modelCode, e.Message);
        //        Console.WriteLine(message);
        //        CommonTrace.WriteTrace(CommonTrace.TraceError, message);
        //    }
